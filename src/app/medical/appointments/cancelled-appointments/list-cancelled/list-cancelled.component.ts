...
export class ListCancelledComponent implements OnInit {
  ...
  private checkAndStartTour(): void {
    setTimeout(() => {
      if (!this.driverTourService.isTourCompleted('cancelled-appointments')) {
        this.startTour();
      }
    }, 500);
  }
  ...
  startTour(): void {
    this.driverTourService.startCancelledAppointmentsTour();
  }
  ...
  searchData(value: string): void {
    ...
    this.appointmentList = this.appointment_generals.filter((item: any) => {
      return (
        ...
        item.motivo_cancelacion?.toLowerCase().includes(lowerValue)
      );
    });
