...
export class ListAreasComponent implements OnInit {
  ...
  addArea(): void {
    this.router.navigate(['/teaching/add_area']);
  }

  editArea(item: CatalogItem): void {
    this.router.navigate(['/teaching/edit_area', item.id]);
  }

  loadAreas(): void {
    this.loading = true;
    this.catalogsService.getAreas().subscribe({
      next: (response) => {
        if (response.success && response.data) {
          this.areasAll = response.data;
          this.totalData = this.areasAll.length;
          this.PageSize();
        }
        this.loading = false;
      },
  ...
  private calculateTotalPages(totalData: number, pageSize: number): void {
    ...
    // reset pageSelection to avoid accumulation on repeated calls
    this.pageSelection = [];
  ...
  confirmDelete(): void {
    ...
    this.catalogsService.deleteArea(this.itemToDelete.id).subscribe({
      next: (response) => {
        if (response.success) {
          this.text_success = 'Área eliminada correctamente';
  ...
  toggleStatus(item: CatalogItem): void {
    this.catalogsService.toggleAreaStatus(item.id).subscribe({
      next: (response) => {
        if (response.success) {
          this.loadAreas();
        }
