...
export class ListModalitiesComponent implements OnInit {
  ...
  /**
   * Inicia el tour guiado de la lista de modalidades
   */
  public startModalitiesListTour(): void {
    this.driverTourService.startModalitiesListTour();
  }
  ...
  toggleStatus(item: CatalogItem): void {
    this.catalogsService.toggleModalidadStatus(item.id).subscribe({
      ...
      error: (err) => {
        ...
        this.translate.get('TEACHING_MODULE.MODALITIES.STATUS_ERROR').subscribe((text: string) => {
          alert(text);
        });
      }
    });
